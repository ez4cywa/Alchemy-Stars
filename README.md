...
> **Avalonia AOT preview branch:** this test line targets .NET 11 Preview 7, Avalonia 12.1.2 and version `1.3.0-preview.22`. The complete desktop workflow now runs as a self-contained Native AOT application. Production remains v1.1.9 on `main`; the existing WPF application is still the stable baseline until .NET 11 GA. Do not distribute this build as a stable release.


Latest changes: [preview.22 update notes (Chinese)](docs/releases/1.3.0-preview.22.zh-CN.md): independent Ubuntu GTK/Yaru-inspired control templates, Gtk material tokens and a complete 34-icon theme sample. Includes all preview.21 fixes.
...
Every functional preview change increments the prerelease revision; this test version is `1.3.0-preview.22`. The stable release remains `1.1.9`.
