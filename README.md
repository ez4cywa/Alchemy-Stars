...
> **Avalonia AOT preview branch:** this test line targets .NET 11 Preview 7, Avalonia 12.1.2 and version `1.3.0-preview.31`. The complete desktop workflow now runs as a self-contained Native AOT application. Production remains v1.1.9 on `main`; the existing WPF application is still the stable baseline until .NET 11 GA. Do not distribute this build as a stable release.


Latest preview release: [preview.31 notes (Chinese)](docs/releases/1.3.0-preview.31.zh-CN.md): integrates ModelMergerGUI 2.2.2 into the left sidebar with grouped CAST merging, two-task scheduling, cooperative cancellation, ammunition filling and independent depth-tested previews. Includes preview.30's online COD database updates. See the [Model merger guide (Chinese)](docs/model-merger.zh-CN.md).
...
Every functional preview change increments the prerelease revision; this test version is `1.3.0-preview.31`. The stable release remains `1.1.9`.
