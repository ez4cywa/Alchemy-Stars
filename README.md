...
> **Avalonia AOT preview branch:** this test line targets .NET 11 Preview 7, Avalonia 12.1.2 and version `1.3.0-preview.24`. The complete desktop workflow now runs as a self-contained Native AOT application. Production remains v1.1.9 on `main`; the existing WPF application is still the stable baseline until .NET 11 GA. Do not distribute this build as a stable release.


Latest changes: [preview.24 update notes (Chinese)](docs/releases/1.3.0-preview.24.zh-CN.md): preserve CAST up axes through both FBX backends, correct Blender-to-Maya unit scaling, and verify animated bones and skin in Maya. Includes all preview.23 timing and workflow changes.
...
Every functional preview change increments the prerelease revision; this test version is `1.3.0-preview.24`. The stable release remains `1.1.9`.
