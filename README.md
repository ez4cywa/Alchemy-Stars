...
> **RF experimental branch:** `EZ4/ravenfield-arms-adapter`, version `1.3.0-rf.3`, independently based on Avalonia AOT preview.27 (.NET 11 Preview 7, Avalonia 12.1.2). Adds Ravenfield pose/full-animation adaptation, a one-file animation library and measured central-palm alignment without changing ordinary exports. Whole-hand contact is not yet guaranteed. Production remains v1.1.9 on `main`. [RF usage guide (Chinese)](docs/ravenfield-adapter.zh-CN.md).
...
Every functional preview change increments the prerelease revision; this independent RF test version is `1.3.0-rf.3`. The stable release remains `1.1.9`.
