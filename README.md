...
> **Avalonia AOT preview branch:** this test line targets .NET 11 Preview 7, Avalonia 12.1.2 and version `1.3.0-preview.17`. The complete desktop workflow now runs as a self-contained Native AOT application. Production remains v1.1.9 on `main`; the existing WPF application is still the stable baseline until .NET 11 GA. Do not distribute this build as a stable release.


Latest changes: [preview.17 update notes (Chinese)](docs/releases/1.3.0-preview.17.zh-CN.md): keyboard shortcuts with F1 help, first-layer output naming, a persistent unified output folder, localized text menus and a centered product title. Users on preview.16 can upgrade with Check for updates; earlier versions require a manual download.
...
Every functional preview change increments the prerelease revision; this test version is `1.3.0-preview.17`. The stable release remains `1.1.9`.
