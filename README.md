...
> **Avalonia AOT preview branch:** this test line targets .NET 11 Preview 7, Avalonia 12.1.2 and version `1.3.0-preview.30`. The complete desktop workflow now runs as a self-contained Native AOT application. Production remains v1.1.9 on `main`; the existing WPF application is still the stable baseline until .NET 11 GA. Do not distribute this build as a stable release.


Latest preview release: [preview.30 notes (Chinese)](docs/releases/1.3.0-preview.30.zh-CN.md): adds an in-app online update for the COD weapon database that re-reads the commit-pinned upstream weapon tables and keeps the existing blueprints. Includes preview.29's COD weapon database page and Call of Duty Wiki reference icons. See the [COD weapon database guide (Chinese)](docs/cod-weapon-db.zh-CN.md).
...
Every functional preview change increments the prerelease revision; this test version is `1.3.0-preview.30`. The stable release remains `1.1.9`.
