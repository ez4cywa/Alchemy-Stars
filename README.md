...
> **Avalonia AOT preview branch:** this test line targets .NET 11 Preview 7, Avalonia 12.1.2 and version `1.3.0-preview.23`. The complete desktop workflow now runs as a self-contained Native AOT application. Production remains v1.1.9 on `main`; the existing WPF application is still the stable baseline until .NET 11 GA. Do not distribute this build as a stable release.


Latest changes: [preview.23 update notes (Chinese)](docs/releases/1.3.0-preview.23.zh-CN.md): frame-rate-correct CAST resampling, collision-safe exports, CAST-only input validation, safer resource workflows and regression coverage. Includes all preview.22 theme changes.
...
Every functional preview change increments the prerelease revision; this test version is `1.3.0-preview.23`. The stable release remains `1.1.9`.
