...
> **Avalonia AOT preview branch:** this source branch targets .NET 11 Preview 7, Avalonia 12.1.2 and version `1.3.0-preview.12`. This revision is source-only; the latest downloadable prerelease remains `1.3.0-preview.11`. Production remains v1.1.9 on `main`, and the existing WPF application is still the stable baseline until .NET 11 GA.
...
Every functional preview change increments the prerelease revision; this source revision is `1.3.0-preview.12` and has no separate GitHub release. The latest packaged preview remains `1.3.0-preview.11`, while the stable release remains `1.1.9`.
