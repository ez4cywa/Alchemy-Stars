...
> **Avalonia AOT 预览测试分支：** 本测试线使用 .NET 11 Preview 7、Avalonia 12.1.2，版本为 `1.3.0-preview.23`，完整桌面工作流已可作为自包含 Native AOT 程序运行。正式版仍是 `main` 上的 v1.1.9；在 .NET 11 GA 前，现有 WPF 程序仍是稳定基线，请勿把本分支构建作为稳定版分发。


本次更新：[preview.23 详细更新日志](docs/releases/1.3.0-preview.23.zh-CN.md)，修复不同帧率动画的时间线换算，增加输出覆盖保护和 CAST-only 输入校验，改进资源删除、剪贴板、路径显示及自动手部默认值，并补充回归验证。包含 preview.22 的 Ubuntu GTK/Yaru 主题改动。
...
项目约定每次功能性改动都迭代版本；本次测试版本为 `1.3.0-preview.23`，稳定版本仍为 `1.1.9`。
