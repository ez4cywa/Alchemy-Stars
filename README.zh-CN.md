...
> **Avalonia AOT 预览测试分支：** 本测试线使用 .NET 11 Preview 7、Avalonia 12.1.2，版本为 `1.3.0-preview.19`，完整桌面工作流已可作为自包含 Native AOT 程序运行。正式版仍是 `main` 上的 v1.1.9；在 .NET 11 GA 前，现有 WPF 程序仍是稳定基线，请勿把本分支构建作为稳定版分发。


本次更新：[preview.19 简明更新日志](docs/releases/1.3.0-preview.19.zh-CN.md)，选中图层后自动显示属性，长名称保留后缀，统一版本及无障碍标签，并保留 preview.18 的全部更新。
...
项目约定每次功能性改动都迭代版本；本次测试版本为 `1.3.0-preview.19`，稳定版本仍为 `1.1.9`。
