...
> **Avalonia AOT 预览测试分支：** 本测试线使用 .NET 11 Preview 7、Avalonia 12.1.2，版本为 `1.3.0-preview.32`，完整桌面工作流已可作为自包含 Native AOT 程序运行。正式版仍是 `main` 上的 v1.1.9；在 .NET 11 GA 前，现有 WPF 程序仍是稳定基线，请勿把本分支构建作为稳定版分发。


最新预览版：[preview.32 详细更新日志](docs/releases/1.3.0-preview.32.zh-CN.md)，修正 idle 手部 IK 的旋转精度，支持换弹动画的 hand mover 目标跟随，并完善按基础动画及鼓匣／扩容弹匣家族自动匹配 pose。保留 preview.31 的[模型合并工作区](docs/model-merger.zh-CN.md)。本次仅编译打包，未运行功能或页面测试。
...
- 动画 CAST 默认只含烘焙动画，不再混入模型、网格、材质或蒙皮；合成预览使用独立的完整场景。模型部件页可另行导出绑定后的手臂、武器与附件模型。
...
下载并完整解压 [preview.32 ZIP](https://github.com/ez4cywa/Alchemy-Stars/releases/tag/v1.3.0-preview.32)，运行 `AlchemyStars.Avalonia.exe`。请保留原生 DLL 与 `Converters` 目录，无需安装 .NET 或 Rust。
...
打开“设置 → 输出”可在 `.cast`、`.fbx`、`.smd`、`.seanim` 中选择默认动画格式。动画 CAST 始终只写动画数据，无需开关；“仅烘焙相关骨骼”可只保留基础动画、姿势、动画层、IK 及间接受影响骨骼的曲线。合成预览始终由独立的完整 CAST 缓存提供，因此仍会显示手臂和武器。

在“模型部件”页点击属性检查器中的“导出绑定模型”，可按动画混合使用的骨架挂接、网格变换和蒙皮权重重映射规则输出手臂、武器与附件。名称为 `武器源文件名_model`；动画格式为 CAST、FBX、SMD、SEAnim 时，模型分别输出 CAST、FBX、SMD、SEModel。
...
“仅烘焙相关骨骼”默认关闭，以保证最大兼容性。把仅动画 CAST 导入已有绑定时，必须使用完全匹配且处于干净绑定姿势的骨架；可先从同一工程导出绑定模型。SMD 因格式要求仍会逐帧写出完整姿势。FBX 需要本机 Blender 或 Maya。
...
项目约定每次功能性改动都迭代版本；本次测试版本为 `1.3.0-preview.32`，稳定版本仍为 `1.1.9`。

要检查某组手臂 + 武器同时满足动画混合与挂点双持，直接传入三个真实素材（不会作为测试夹具提交）：

```powershell
AlchemyStars.Avalonia.exe --asset-pair-smoke <手臂.cast> <武器.cast> <动画.cast> <输出目录>
```

它会验证分类是否正确、合并后顶点与骨骼数量是否守恒、武器根是否正确挂到解析出的挂点骨骼（单持为 `<根名>__weapon`，双持为 `<根名>__left`/`<根名>__right`）、骨架是否单根且骨骼名不重复、两条管线烘焙出的帧数是否一致、每份产物只包含应有的内容（几何在绑定模型、动画在 CAST），以及源文件是否保持字节不变。
