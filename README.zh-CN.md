...
> **Avalonia AOT 预览测试分支：** 本测试线使用 .NET 11 Preview 7、Avalonia 12.1.2，版本为 `1.3.0-preview.31`，完整桌面工作流已可作为自包含 Native AOT 程序运行。正式版仍是 `main` 上的 v1.1.9；在 .NET 11 GA 前，现有 WPF 程序仍是稳定基线，请勿把本分支构建作为稳定版分发。


最新预览版：[preview.31 详细更新日志](docs/releases/1.3.0-preview.31.zh-CN.md)，左侧新增「模型合并」：整合 ModelMergerGUI 2.2.2 的多组合并、双任务队列、协作取消、弹匣填弹和独立深度预览；保留 preview.30 的在线武器库更新。用法见 [模型合并工作区](docs/model-merger.zh-CN.md)。
...
项目约定每次功能性改动都迭代版本；本次测试版本为 `1.3.0-preview.31`，稳定版本仍为 `1.1.9`。
