...
`1.3.0-rf.5` 是基于 `1.3.0-preview.27` 的独立实验分支 `EZ4/ravenfield-arms-adapter`。普通 CAST/FBX/SMD/SEAnim 导出保持原流程；RF 适配使用独立入口和米制 FBX 预设。
...
三个实际虎口点加四个掌面点联合拟合。左掌在参考握持姿态中建立武器三角形锚点，四个目标点使用 0.7 mm 接触偏移；源手离枪距离从 6 mm 增加到 24 mm 时平滑释放，不逐帧重选最近三角形追枪。手动位置/旋转在目标计算后应用。
