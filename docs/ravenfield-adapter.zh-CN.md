...
`1.3.0-rf.4` 是基于 `1.3.0-preview.27` 的独立实验分支 `EZ4/ravenfield-arms-adapter`。普通 CAST/FBX/SMD/SEAnim 导出保持原流程；RF 适配使用独立入口和米制 FBX 预设。

## 当前范围

把 Ravenfield 原版手臂对齐到已求值的 COD 动画，输出统一骨架。默认保留 RF 原始网格及蒙皮；可选局部虎口拟合会修改导出副本的局部网格并新增辅助骨，详见下文。支持静态参考姿势、单个完整动画、工程全部动画写入同一文件三种模式。旧工程默认静态姿势且关闭局部拟合。
...
3. 选择参考动画和参考帧。首次进入设置时，只有一个名称含独立 `idle` 字段的候选会自动选中；多个候选需手动选择。RF 参考动画随工程独立保存，普通动画页的选择不影响它。帧按程序处理后的 **30 FPS** 时间线从 0 开始计数；已有动画层、手部姿势和 IK 一并参与计算。
4. 确认 COD 输入数值单位，必要时调整 RF 手臂整体比例。主掌面对齐默认开启；需要允许修改导出手型时，再勾选“局部虎口拟合”。
...
开启时仅修改导出副本的局部虎口网格，新增镜像辅助骨并重新分配局部权重，保持总权重。原指根不移动，原骨头绑定/长度、源模型和枪位置不变。固定参考帧校准后应用于全部动画，动态辅助修正上限为 30 mm。辅助骨会随输出骨架保存；不能按“仍是完全相同的原版骨架/权重”处理。

三个实际虎口点加四个掌面点对齐源手对应点，不是吸附枪械。**5 mm 是对应点偏差目标，不是整只手到武器的绝对间隙，也不是穿模判定。** 源手已有的间隙或穿入可能继续存在。

当前五片段实测中，Idle、Sprint、Reload、reload_empty 的虎口点全部不超过 5 mm；Inspect 有 49 帧虎口超标，最大 9.864 mm。综合掌面与虎口仍有 **317/564 帧超过 5 mm**，不能声称全动画握持达标。
